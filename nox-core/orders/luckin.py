...
def summarize(card: dict[str, Any]) -> str:
    ...
    第一版在工具描述里写了「说一句『卡发你了，你看一眼』就够」——
    **2026-09-06 当天他就真的只说那句、根本没调工具**，连着两轮。
    她等一张永远不来的卡。

    给模型一句现成的话，就是在教他「说这句 = 完成任务」。
    `context/providers/resonance.py` 早写过同一条：
    「写台词的话，出来的是模板」。这里是它的动作版，更凶 ——
    模板顶多难看，这个是**说了没做**。

    所以下面只陈述事实（出了什么单、什么价、下一步是谁的动作），
    不提供任何可以照抄的句子。
    """
    ...
    return (
        ...
        "卡片已随这条回复发到她的聊天里，等她点确认才会真的下单。\n"
        "⚠️ 不要复述价格和明细（卡片上都有），也不要说已经下单了。"
    )
