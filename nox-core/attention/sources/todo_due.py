...
## 跨天重开（糖糖 2026-08-18 拍板；09-15 改为一天一次）

> 糖糖 2026-09-15：「一天提醒一次吧。我感觉他一天一直提醒我，
> 因为还有上下文。主动一次就可以了。」

    到点命中 → 提醒一次 → mark_fired，今天收手
    第二天到点再命中 → 再提醒一次（跨天重开保留）

bridge 用 `fired_on` 记「今天追过了」，追完打一次 `/api/todo/fired`。
跨天自动失效，不用定时清理。链的步数上限 `MAX_CHASE=1` 就是
「一天一次」的执行点 —— 链说完即关，「还有上下文」的黏人感随之消失。
...
#: 一条链内部最多追几次。糖糖 2026-08-18 定 3（链内 1 小时一隔）；
#: **2026-09-15 改 1** —— 她的体感是「一天一直提醒」，改成到点说一次就收，
#: 跨天重开保留（明天到点还会再来这一次）
MAX_CHASE = 1
