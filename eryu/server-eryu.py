...
class EryuHandler(BaseHTTPRequestHandler):
    ...
    def _handle_music_remote_get(self):
        ...
        f.unlink()
        # 兼容旧格式：以前这里存的是**扁平的一首歌**，没有外层包装
        if "song" in data:
            payload = {"ok": True, "song": data.get("song")}
            if data.get("queue"):
                payload["queue"] = data["queue"]
        else:
            payload = {"ok": True, "song": data}
    ...
    def _handle_music_remote_post(self, body: dict):
        ...
        if not song:
            ...
            return
        # ⚠️ 要**整包存**，不能只存 song。
        # 原来写的是 `f.write_text(json.dumps(song))` —— 把 body 里的
        # queue 整个丢掉了，所以 Core 明明排好了 10 首，前端永远只收到一首
        # （2026-08-10 查出：Core 日志说「后面还排了 6 首」，
        # 队列文件里却 queue=0）。
        record = {"song": song}
        queue = body.get("queue")
        if isinstance(queue, list) and queue:
            record["queue"] = queue[:20]
        f = self.state.data_dir / "music_remote.json"
        f.parent.mkdir(parents=True, exist_ok=True)
        f.write_text(json.dumps(record, ensure_ascii=False))
        self._send_json(200, {"ok": True})
