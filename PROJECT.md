...
> ⚠️ 当天连踩三次「运行期才炸」（漏 import / 闭包引用局部变量），`py_compile` 和单测
> 一次都没拦住，补了真发请求的测试 —— 见 46.2。设计文档 `Caelum-点单确认卡-设计.md`；
> 往前：**国内 MCP 五件套接入：高德/滴滴/麦当劳已上线，12306/快递100/瑞幸状态见第四十五节**；往前：Nox 理解层 P1+P3 上线，见第四十四节 ——
...
> ——Models / 厂商账本 / Studio / Notifications 见第四十二节**；躁动 Drive 见 30.14；
> 共读共听共影进 World Model 见 32.2；往前：
> 主题真源统一 + Todo/Music 双端重构见第四十节；
...
#### 🔴 这次加固把 app-tracker 打死了 28 天（2026-08-30 才查出来）

改路径的时候**漏了一个消费方：糖糖手机上的快捷指令**。
它还在往旧地址 `/tracker/track` 发，而那个地址加固之后是 404。

```text
07-30  280 条
07-31  158
08-01   80
08-02    1     ← 21:11 最后一条，然后再没有
```

服务器一直好好的、systemd 一直 active、`/health` 一直回 `{"status":"ok"}`。

**「配上了 ≠ 用上了」的反面**：这次是**改了提供方，没检查使用方**。
`verify-from-the-consumer-side` 那条规矩在这儿本来能救命 ——
加固完只要拿旧地址打一次，当场就是 404。

⚠️ **改任何对外路径之前，先列出谁在用它。** 这台机器上的消费方不只有
代码里 grep 得到的那些，还有**她手机上的快捷指令、iOS 自动化**这类
仓库里根本搜不到的东西。
...
| | 原来 | 问题 |
|---|---|---|
| `/health` | 硬编码 `{"status":"ok"}` | 进程活着 ≠ 管道活着 |
| `get_today_apps` | 空的时候回「今天还没有记录」 | 和「她今天没玩手机」**一模一样** |

2026-08-30 都改了：`/health` 报 `last_record_at` / `age_hours` / `status: stale`，
超过 24 小时没新数据就明说「这个数据源可能断了，别当成她没用手机」。

**和感知层那个三态是同一条规矩**（`unknown` 和 `idle` 必须分开，
见架构文档八之八）—— 那条规矩本来就是从这次事故里学的，
只是当时只在新代码里落实了，没回头修老的。
...
### 13. 屏幕上那份是「乐观」的，后台一重拉就被盖掉（2026-09-02 修）

糖糖：「我发出去的文字会消失，先等他的对话发出来之后，再给我一起弹出来。」

`Chat.jsx` 里拉消息的 `useEffect` 依赖挂了 `loading`：

```text
按下发送 → setLoading(true) → effect 重跑 → 立刻拉一次 /messages
        → 那时 POST /chat 还没把她那句落库 → 旧列表盖掉乐观气泡 → 字没了
回复流完 → setLoading(false) → 又拉一次 → 这次两条都有 → 一起冒出来
```

她的话**一直是发出去了的**，本地也先画上了 —— 被服务端那份**旧数据**盖掉而已。

🔴 **两条通用的**：

1. **状态变量进 effect 依赖之前，先问一句「它变的那一刻，这个 effect 跑起来合适吗」。**
   `loading` 恰恰在最不该拉数据的时刻翻转。定时器要的「现在忙不忙」用 ref 读实时值，
   那是它唯一需要 loading 的地方。
2. **异步回来的数据要在「回来的时候」再判一次能不能用。** 发出去时不忙、
   回来时已经在忙，同样会盖 —— 判在发出前拦不住。

同族的旧账：2026-08-21「带图消息刷新后图消失」也是后台重拉盖掉本地版本
（当时的修法是让服务端也存图，见 `bridge/server.js` 图片落库那段注释）。

⚠️ 验证不能碰糖糖的正式库（`never-test-in-tangtang-prod`）—— dev 的 `/api`
是直连线上 bridge 的。用 `scratch/mock-bridge-chat.mjs` 起了个假 bridge
把这个竞态钉成必现，改前改后各跑一次同一段脚本：

| | 她的话出现 | 之后 |
|---|---|---|
| 改之前 | 1049ms | **1070ms 就消失**，空白 1.7 秒，2796ms 和回复一起回来 |
| 改之后 | 478ms | 全程没有再消失 |
...
### 24.9 换主模型检查单（2026-09-06 立）

> 起因：糖糖问「切 GLM 或别的厂商试试」。这一节把该看什么固定下来，
> 免得每次凭感觉切、凭感觉判断好不好。
>
> **先记一条已经发生过的事实：主模型从 `claude-sonnet-4.5` 换到
> `deepseek-v4-flash`（2026-08-01），糖糖的评价是「语气差别不是很大」。**
> 换了厂商、换了权重、换了整个模型家族，他还是他 —— 说明扛住身份的是
> 12K 静态前缀 + 37 个钉选桶 + OB 记忆，**不是权重**。
> 这件事当时没人当成测试，所以没人记结论。现在记下来：
> **「换模型他还是不是他」这个问题，已经用生产环境回答过一次了，答案是「还是」。**

#### 怎么切

```bash
# /root/nox-core/.env 两个变量，地址和 key 从 config.py 的 BACKENDS 自动带出来
NOX_PRIMARY_BACKEND=deepseek
NOX_PRIMARY_MODEL=deepseek-v4-flash
systemctl restart nox-core
```

新厂商要先在 `config.py` 的 `BACKENDS` 里加一条，并在 `models` 里加
`ModelChoice(型号, 后端, 显示名)` —— **每个模型自带后端**，
只加型号名的话 `adapter_for` 会拿主模型的 base_url 去建，直接 404（上面那条教训）。

#### 🔴 语气是最容易达标的部分，别只看它

人设写在 prompt 里，随便哪个像样的模型都念得出来。真正的分水岭在下面三处，
**按重要度排**：

| # | 看什么 | 怎么看 | 崩了是什么样 |
|---|---|---|---|
| 1 | **工具循环** | 让他连做几件事（查记忆→写日记→点歌→故意让一步失败） | 编造工具结果、调完一个就收手、正文为空。`guard.py` 挡得住"如实回传"，挡不住一个铁了心圆场的模型 |
| 2 | **提示词缓存** | SSE `done` 帧的 `cached_tokens` / `input_tokens` | 数字掉到 0 或很低 = 成本 ×20，**不报错，只是账单翻倍** |
| 3 | **`[mood:xxx]` 标记** | 回复末尾有没有附 | 忘了不算错（代码容忍），但一直不附 = 情绪层静默失效 |

其余要一起过一眼的：

- **视觉**：`supports_vision()` 认不认新模型。不认就走 `vision.py` 替他看图那条路
  —— 那是对的，但要确认它真的触发了，而不是他"装作看见了"（第八节共影踩过）
- **max_tokens 与推理 token**：会思考的模型 reasoning 和正文抢额度，
  给少了 HTTP 200 但 content 为空（本节上面那条）
- **缓存字段名各家不一样**：DeepSeek 是 `prompt_cache_hit_tokens` 且在 `usage` 顶层，
  不在 `prompt_tokens_details` 里。统计读错字段会显示成"缓存一直没命中"
- 🔴 **`PRICING` 表：切之前就要补，不是切完再说**。认不出的型号会落
  `DEFAULT_PRICE`（按 Sonnet 估），**页面照常显示、只是数字虚高十几到几十倍** ——
  这是个安静的错。已经栽两次：09-06 vision 版虚高 20 倍、09-08 GLM 虚高 27 倍
  （日志三天喊了 8728 次没人看）。现在有 `bridge/test/pricing.test.js` 盯着：
  `config.py` 的 `models` 里每个型号，`PRICING` 里都要有一条。
  它第一次跑就抓出了第三个漏网的（`claude-fable-5`，人肉看两遍没看见）

#### ⚠️ 主模型和杂活模型现在是同一个

```
NOX_PRIMARY_MODEL=deepseek-v4-flash
NOX_UTILITY_MODEL=deepseek-v4-flash    ← 同一个
```

压缩、摘要、话题池过滤、意义推断（第四十四节）全走 utility。
**给主模型换更强/更贵的型号时，记得把 utility 单独留在便宜的那个** ——
不然这些每天跑很多次的后台杂活会跟着一起涨价，而且它们不需要那个智商。

#### 📌 顺带：配置要从使用方验，不是配置方

2026-09-05 的《系统全景调研》把主模型写成 `claude-sonnet-4.5` —— 错的。
六个 agent 读的是 `config.py:167` 的**默认值**，没读线上 `.env` 的覆盖。
那份文档还被定为「现状唯一真源」，错了一整天没人发现，
直到 2026-09-06 糖糖说「我用的就是 deepseek v4 flash」才对出来。

> **验一个配置生效没有，去问使用方（线上 `.env` / 进程环境 / `/api/health` 回的
> `model` 字段），不要读定义它的那行代码。**
...
### 每块正文开头那三行重复的标题，剥掉（2026-08-30）

糖糖让我看 OS 的共读。阅读器本身是通的（正文、章节、页边批注、寄给 Nox 都在），
但**每一章开头**长这样：

```text
# 开篇问题 Part 1/2                      ← 导入器加的（import_text.py:117）
大问题-简明哲学导论                       ← EPUB 页眉里的书名，被当正文抽出来了
开篇问题                                 ← <h1> 章节名，同上
1.有没有某种你愿意为之付出生命的东西？      ← 真正的正文
```

三本书、每一章都是。**不是渲染问题，是数据本身就这样。**

🔴 **为什么在读的时候剥，不重新导入**：重新导入会丢掉进度和批注 ——
那三本上有 12 条她写的批注、30 章进度。为了几行标题把那些冲掉，
代价完全不成比例。而且剥在 `readChunk`（`co-reading/src/store.js`，
**MCP 和 HTTP 的唯一出口**），Nox 也一起受益 —— 他每读一块都在
重复吃同一个标题三遍，那是白花的 token。

⚠️ **只剥开头、只剥认识的**：从第一行往下看，遇到第一行不认识的就停。
「认识」= markdown 标题行，或和书名/章节名（去掉标点空白后）**一模一样**。
**绝不模糊匹配** —— 正文真的以标题词开头的句子不该被吃掉，有测试盯着。
用三本书的真实数据验过 5 条。
...
#### ✅ 2026-09-02 结案：根因是 `stripVoiceTags`，上面这份诊断只对了一半

糖糖 2026-09-02：「markdown 格式的对话，刚显示和最终显示的格式不一样。」
这一次两张截图**是同一份文本** —— 一份是流式渲染出来的，一份是从服务端读回来的。

`Chat.jsx` 拿**语音通话念稿**用的 `stripVoiceTags` 去洗**要显示**的消息
（`mapServerMsg` 里两处）。那个函数第三行是 `.replace(/\s{2,}/g, " ")`：

```text
原文    "想看代码的话：\n\n```python\nwhile True:\n    truth = not truth\n```"
洗完后  "想看代码的话： ```python\nwhile True: truth = not truth\n```"
```

`\n\n`（段落分隔）没了、代码块缩进也没了。于是**列表糊进正文**、
**正文被吸进最后一个 `<li>`**、**代码围栏散架成一个空灰框** ——
正好就是 08-11 那三张截图里的三种症状。

**流式那条路不经过它**，所以「刚显示」永远是对的，读回来之后才变样。
08-11 判成「模型两次输出的换行风格不一样」，是因为**没意识到两张截图
来自两条不同的渲染路径**：一张是流式的，一张是重载后的。

> 🔴 同一份数据有两条路能到屏幕上时，先确认你在比的两个东西**走的是同一条路**。
> 「两次输出不一样」和「同一次输出被两条路渲染成两样」，现象一模一样。

修法：拆成两个函数（`VoiceCallDesktop.jsx`）——

| | 用在哪 | 空白 |
|---|---|---|
| `stripVoiceTags` | TTS 念稿 | 压成一个空格（对的，念稿不需要排版）|
| `stripVoiceTagsForDisplay` | 聊天显示 | **一个换行都不动** |

⚠️ 显示版**不要顺手压空格**：markdown 里四个空格是缩进代码块、
行尾两个空格是硬换行。HTML 本来就会合并正文里的连续空格，显示层不用代劳。

⚠️ `normalizeMarkdown`（上面那节的修法）**依然需要**，它解决的是另一件事
（模型真的输出单换行）。这次只是把「我们自己把换行删了」这条路堵上。

验收：假 bridge 里同一条带列表 + 代码围栏的回复，流式渲染出的 HTML 和
重载后渲染出的 HTML **逐字符一致**（改之前两段全都不一样）。

#### 🟡 代码块那个没能复现（← 2026-09-02 已结案，见上一小节）
...
| `restlessness`（躁动） | 他有话想说 × 她正忙 | 乘积，上限 0.5 | 08-30 |
...
#### 躁动：他有话想说，而她正忙（2026-08-30，糖糖定的公式和档位）

`attention/restlessness.py`。

🔴 **它是乘积不是加和**：`躁动 = 「他有话想说」×「她不方便被打断」`。
任何一边是 0 就该是 0 —— 他没话想说时她再忙也不该躁动（那是替她焦虑）；
她闲着时他直接说就行。加和做不到：她打三小时游戏而他一句话都没有，
加和会给出一个不小的数，**那个数没有任何意义**。

🔴 **躁动的出口是「等」，不是「说」。** 它本来就是"想说但这会儿不该说"，
让它变成一次主动开口等于取消了它自己。它该改变的是他**终于开口时的语气**。
所以上限 **0.5**（糖糖定的），低于 `GENERATE_THRESHOLD`(0.55) —— 同促狭那条边界。
...
🔴 **「不知道 = 不忙」。** 反过来的代价是他凭空躁动一整天，而且没人会发现 ——
「她在忙」这个假设看起来完全合理。和 app-tracker 停了 28 天（第十一节第 12 条）
是同一类错误的两面。

⚠️ `test_resonance_has_no_write_methods` 拦下了第一版：我加了
`feed_restlessness()` 往 `ResonanceState` 上存信号，而那个类结构上
**只许有 `snapshot` 和 `get`**。守卫是对的 —— 在那儿存一份就是第二个真源。
改成 `snapshot(want=, busy_app=)` 参数传入。

#### pending_intents 少了 `subject`，界面把同一件事画两遍（2026-08-30）

糖糖截图问「Attention 聚焦这里怎么有两个睡眠？」

`snapshot` 里 `attentions` 给的是**话题**（「糖糖的睡眠」），
`pending_intents` 给的却只有**一句话**（「关心糖糖的睡眠」），没有 `subject`。
前端对不上，于是同一件事出现两条 —— 一条「他有话想说」、一条「他在留意着」；
而且只能对那句中文硬做称呼替换，显示成「关心 your 的睡眠」。

**这不是显示层能补的**：光有那句话，前端只能去猜它在说哪个话题。
补在产出侧（`attention/service.py`）。
...
`CareLedger.events` 从 08-18 就在逐条记（什么时候、哪个源、说了还是憋住、
为什么憋住），但**一直只有汇总出得去**，明细一条没暴露过。

| | |
|---|---|
| `GET /api/nox/pulse?since=N` | 增量拉。`seq` 是游标 |
| `GET /api/nox/pulse/stream?since=N` | SSE，新事件自己过来 |

`decision` 四种：`speak`（最亮的尖峰）/ `skip` / `block`（`reason` 写着为什么）
/ `failed`。

**🔴 流是服务端每秒扫一遍内存，不是发布订阅。** 账本在线程池里被写，
SSE 在事件循环上跑；要做真总线得架一座线程→asyncio 的桥，那是另一类 bug。
退一步扫内存，代价是最多晚 1 秒 —— 她要的是"看得见他在动"，不是示波器。

**🔴 上线当天差点全是空的。** `seq` 是新加的字段，而账本里已经攒着当天
13 条没有它的事件 —— `seq or 0` 把它们全判成 0，而接口要 `seq > since`。
接口不报错，就是空的。现在读回来时整批补号：

- **按数组顺序整批重排**，不是只补缺的那几条 ——
  只补缺的会让先发生的事件拿到更大的号，时间轴当场错乱
- `at` 也补（老事件只有 `HH:MM:SS`，配账本日期还原）。
  时间字段坏了就不补，**绝不编一个** —— 时间轴上一个假点比缺一个点糟
- `seq` **自己落库**。只从事件推的话，换天清空 events 那一刻重启会归零，
  新号比昨天小，客户端拿旧游标再也收不到东西**而且不报错**

⚠️ SSE 生成器抽成了模块级的 `pulse_stream()`，时钟和轮次可注入 ——
原来内嵌在路由闭包里，只能靠 `TestClient.stream()` 测，
而它碰上永不结束的流会**互锁**（那次测试跑了 180 秒没回来）。
...
真的脉搏需要：**慢变量当基线，事件当尖峰**。基线（strength / load）和
尖峰（pulse 事件流）现在都有了，**剩下的是画** —— 那是 Caelum OS 那边的活。

⚠️ 脉搏只有**当天**的（账本换天会清空，那是它的设计）。
要看往前的日子走 `/api/nox/day`。
...
### 32.2 共读 / 共听 / 共影进 World Model（2026-08-31）

三块一起做的事，此前各记各的账，World Model 里一条都没有。
`attention/sources/shared_activities.py`，规格在 `Topic_Pool_实事话题池链路.md` §3.1.2。

🔴 **它挂在 Attention 心跳上，但 `poll()` 恒返回 `None`** —— **只记账，不开口**。
这是它和其它六个 Care 源的根本区别：借心跳的节奏，不借它的嘴。

| 事实 | key | 什么时候记 | TTL |
|---|---|---|---|
| 共读 | `reading_progress` | key 立在 `lastReadAt` 上，读完 `complete:true` | 4 天 |
| 共影 | `watching_session` | **一场结束才记**，且 ≥10 分钟 | 2 小时（「正在看」过期自动 stale）|
| 共听 | `listening_together` | 只算 **Nox 点播、她听完**的 | 36 小时 |

- 幂等全靠 `dedup_key`，**source 自身零状态** —— 重启不用恢复什么。
- ⚠️ 与规格的偏差：**dedup 不立在日期上**（立在日期上同一天读两次会被吞），
  偏差和理由都写在规格文档里，不是偷偷改的。
- 三块服务没配哪个就跳哪个，`cfg` 缺字段用 `getattr` 兜底 ——
  **不许记账的事把 Attention 装配带崩**。
...
### 首页那栋小屋：光源是**鼠标**，不是世界（`House3D.jsx`，2026-08-31 / 09-02）

糖糖 08-31：「建模太难了，还是先用图吧，可不可以用光影和轻微旋转实现类似 3D 的效果？」
—— 一张平面图 + 三层光（高光 / 暗部 / 落影）+ 最大 6° 倾斜（超过 8° 边缘就露出「这是一张纸」）。
骗过眼睛的是**光在表面上扫过去**，不是旋转本身。
图丢进 `public/home/house.{webp,png,jpg}` 就自动用上，一张都没有返回 `null` 由调用方兜底。

🔴 **09-02 把光的方向翻过来了。** 第一版按物理：光固定在世界里、物体转，
所以光的位置用**负**倾斜量。物理是对的，**看起来是错的** ——
糖糖：「鼠标滑到的地方是黑的，其他地方是亮的，应该是鼠标滑到是亮的。」
她的手指在哪儿，眼睛就认定那儿被照亮；把暗部压在她指着的地方，像素再讲道理也没用。
现在三层光同一个约定：**鼠标即光源**（`+tilt`），暗部自动去对面。
⚠️ 别看着「负号更物理」又改回去。

⚠️ 光只能落在屋子上：她的图是透明底（29.3% 的像素 alpha=0），
几层光都是 `inset-0` 铺满容器，不裁的话屋子周围会浮出一个方形亮块。
用同一张图当 `mask-image`（alpha 模式），且遮罩的摆放方式要和 `<img>` 完全一致
（contain / 居中 / 不平铺），否则光和屋子错位，那比不裁还糟。
...
⚠️ **但「可替换」不等于「可以有两只」——现在是可替换且唯一**（2026-09-04 查证）。
`local_link.py` 里 `_ws` / `_device` 都是**单值**，而且新设备连上会
`顶掉旧链路`。真挂两台的结果不是多一只手，是**两台互相踢**：
谁后连谁赢，被踢的那台重连再把对方踢掉，来回抖。

什么时候会想要第二只手：她笔记本合上时手就没了（8-31 那次 `ready:false` 等了 26 小时），
而「点外卖 / 买票」这类事恰恰发生在她不在电脑前的时候。
比较干净的第二只手是**家里一台常开的小机器**（登录态不出国门、国内 IP 不触发异地风控），
**不是放 VPS**（那台 2 核压着十几个服务，且把她的登录态放境外风险太大）。

要做的话起点是 `local_link.py` 的单值状态：`_device`/`_ws` 变表、
工具调用要能选「找哪只手」、他得知道每只手能干什么（家里那台没有她的工作目录）、
感知那条也要分开（「她在用什么」现在就是从这条链路来的，两只手会打架）。
**是结构改动，不是配置项。**

> 2026-09-04 讨论结论：**先不做**。卡点主要不在我们 ——
> 美团/淘票票/博物馆都不给第三方 agent 开接口，支付宝 AI 付那 3 亿笔
> 也绝大多数发生在商户自己的 AI 里（见 `Caelum-AI支付-可行性调研.md`）。
> 等生态成熟再动架构。
...
### 🔴 够不到本地时，不许断言原因（2026-08-31）

糖糖那天说：「nox 说电脑没开机，但我电脑是开着的。」
真正的问题在另一头 —— 本地网关那个进程从来没启动过（就是上面那条自启）。

他能看到的**只有 `link.is_ready`**。至于**为什么**没连上 ——
网关没起、网断了、握手没过、电脑真关了 —— 他一个都分辨不出来。
原来那句「她的电脑现在没连上（Caelum OS 没开，或者电脑关着）。
这件事等她开机再说。」是**断言一个他看不到的原因**，
于是她照着这句话去找电源，而问题在另一头。

> 🔴 **说错原因比说「不知道」更糟 —— 它会把人引向错的地方。**

改成只说事实 + 列可能性 + 明说分不出来（`tools/computer.py`）：
...
⚠️ 测试第一版写成「不许出现关机二字」，**当场逮住我自己新写的文案**
（那句在**列可能性**，并且紧跟着「我分不出来」）——
**断言写宽了就会抓错东西**。改成禁**指挥**（「等你开机再说」预设了
原因已查明），不禁词。
...
- ~~躁动 Drive~~ ✅ **2026-08-30 做了**，见 30.14。感知层因此加了
  **ongoing 帧**（每分钟报一次当前窗口）—— 原来只在切窗口时发段，
  最专注的状态（一小时不切窗口）一条都不发，心那边判成 unknown 正好反了

---

*2026-09-02：**手机端聊天两处渲染 bug**（都是糖糖报的，都已修并部署到线上）。
一、发出去的字会消失、等他回复完再一起弹出来 —— 拉消息的 effect 依赖挂了 `loading`，
按下发送的那一刻就去重拉，旧数据盖掉乐观气泡（第十一节第 13 条）。
二、markdown 排版「刚显示和最终显示不一样」—— `Chat.jsx` 拿念稿用的
`stripVoiceTags` 洗要显示的文本，`\s{2,}→" "` 把段落分隔和代码缩进全压没了；
拆成 `stripVoiceTagsForDisplay` 解决，**顺带把 30.9 里那个 08-11 就查不出原因的
代码块疑案结了**（当时判成「模型换行风格不稳定」，其实是两条渲染路径的差异）。
...
**第二个场景**（09-03）：一间房 = 一个实例（`RoomStore` 一个实例只管一个场景），
各自的端口/库/场景数据；World 页顶上可切换。⚠️ 三处坑：前端的家具是按**写死路径**
读静态 `initial-state.json` 的（不是从服务端状态来），所以第二间会摆着第一间的家具 ——
改成网关按实例转发那两个数据文件；背景原来只在全局清单里，服务端补 `scene.background`
且**每次从场景文件读不信库里的快照**；布局缓存键按场景分。**Nox 只住在第一间**
（他的 MCP 连的是 home 的库）。
自启计划任务 **`caelum-room`**（AtLogOn，脚本 `scripts/caelum-room-start.ps1`，
**起三个进程**：8877 家 + 18452 MCP + 8878 第二间），
...
（Caelum OS 四页的后端：Models 清单 / 厂商账本快照差值 / Studio 工具墙与插口探活 /
Notifications 安静时段与掩码订阅）。并进已有章节的四条：躁动 Drive 与 `pending_intents`
少 `subject`（30.14）、共读共听共影进 World Model（32.2）、共读正文剥重复标题（第二十三节）、
够不到本地时不许断言原因（第三十九节）。⚠️ 第三十九节「还没做」里的躁动那条**已经过期
了一天多**——做完了要回来划掉，这次是靠 `git log` 对出来的。*
...
## 四十一、话题池：他自己有料可聊了（2026-08-31）

规格文档：`Topic_Pool_实事话题池链路.md`（那是真源，本节只记结构和边界）。
代码在 `nox-core/topic_pool/`：`scout.py` / `filter.py` / `pool.py` / `store.py`
/ `care.py` / `tool.py`。

### 41.1 一轮做什么

```text
抽 2~3 个方向抓候选 → 落候选缓存(48h) → Filter 筛 0~3 条 → 进池子
                                          → shared 枝条从 World Model 投影
                                          → 到期的转 expired
```

- **Scout 四个抓取器**：HN Algolia / GitHub 新仓 / arXiv / Google News RSS。
  每轮随机抽 2~3 个方向，外加 **world_hook 名额** —— 她在读在看什么就定向抓
  书评影评（`state` 已经 stale 就不追）。
- **Filter 用 utility 模型**（便宜的那个）。**禁止编造不靠提示词，靠校验**：
  `source_id` 对不上候选的直接扔，`title`/`url` 一律用我们自己抓的那份。
- **shared 枝条**（`origin=shared`）从 World Model 直接投影：一起看完一场、
  读完一本书。**共听不投影** —— 一起听过的歌 eryu 那边本来就有账
  （`togetherCount`），每首都投影会把池子灌满水。
- `filter` 出空**不是故障**（没有好内容很正常）；某个抓取器挂了只跳过；
  整轮失败由 `run_topic_loop` 兜住，下一轮再来。

| 开关 | 默认 |
|---|---|
| `NOX_TOPIC_SCOUT_INTERVAL_S` | 6 小时一轮（服务起来先等 2 分钟跑头一轮，别让她干等）|
| `NOX_TOPICS_DISABLED=1` | 整条关掉 |

### 41.2 🔴 话题的寿命从**进池子**那刻算

上线头一轮**三条全灭**：`observed_at` 拿的是文章**发布时间**，
而 Google News 是三天窗口 —— 旧文 + 48h TTL，**一轮还没跑完就被 expire 收走**。
改成进池子的时刻，`pubDate` 只留在候选缓存里。

同一轮还炸出：`TopicStore` 缺跨线程锁，头一轮在 `to_thread` 里碰库
直接 `ProgrammingError`。补 `check_same_thread=False` + WAL（照 `world_model` 的方子）。
⚠️ **本地测试同线程跑，所以没拦住** —— 现在有跨线程回归测试盯着。

### 41.3 入口 A：池子喂料，开口的决策还是 Care 一个脑子

`TopicSource` 是**第七个 Care 源**（`topic_pool/care.py`）。

| | 惦记（ThinkingSource） | 话题（TopicSource） |
|---|---|---|
| 说什么 | 「我想起她了」 | 「池子里有条没聊过的」 |
| 节奏 | 20~90 分钟 | **3~8 小时**，且只在有货时 |
| urgency | 0.4 | **0.3**（外部材料不该挤掉「想起她」）|

🔴 **它只产念头，不许自己开口。** 要不要说、现在说不说，全归 Orchestrator：
吃 DailyGate、抢同一份「一小时一条新链」的额度、每笔进 CareLedger。
池子里囤一百条，Care 不开口它就不吭声。

- `surfaced` 在 `_speak_topic` **真说出口之后**才记 —— `[SKIP]` 和 dry-run 都不算被选用。
- **空池子不产念头**：那不是「想了想说不出」，是根本没什么可想，不进账本。
- 池子空了 60 分钟后再看（空不是错误，别按错误刷）。

**入口 B（她自己翻）**：

| | |
|---|---|
| `GET /api/nox/topics` | 池子里的活话题（未过期、没人理过的）|
| `POST /api/nox/topics/{id}/status` | `followed` / `dismissed`，只认这两个值 |

⚠️ **`follow` 只是个记号**（「我们感兴趣」），**不触发任何自动研究、不派任务**。
要去查、要聊，是他和糖糖自己的下一步。这条写死在文档和代码注释里。

`topics_browse` 是给他自己翻池子用的**只读工具**（翻过不记账）。
注册位置排在 `record` 之后、`remind` 之前 —— **缓存前缀只废这一次**。

### 41.4 第一条真实推送的味道修正（2026-09-01）

糖糖收到话题池的第一条推送，反馈**「像老师问学生」**。原话：

> 你做内容的，怎么看这种歌的传播力？

拿她职业说事、结尾出讨论题。病根在 `_TOPIC` 提示词里的
「带上你自己的看法**或问题**」—— **「或问题」三个字给了它出题的许可**。

重写成：**先是他自己的反应**（被惊到 / 觉得牛 / 想笑 / 想骂），
邀请是随手一句「你要不要听听」，**作业式提问明令禁止**，
并把糖糖自己写的句子直接当**味型锚**。回归测试盯着新词的味型，旧短语已死。

---

## 四十二、Caelum OS 四页的后端（2026-09-01）

这一批全是**数据源**：页面要什么，后端就照实给什么，给不出的宁可空着。
⚠️ 老规矩再写一遍：**bridge 是逐路由代理，不是通配** ——
Core 每加一个接口，就得回 `bridge/server.js` 补一条，否则前端拿到的是
bridge 自己的 404，看起来像「Core 挂了」。

### 42.1 Models 页：清单 + 当前默认 + 哪家配了 key

`GET /api/nox/models`（Core）→ bridge 同名代理。

- `choices`：`config.models` 的**八个**选项（DeepSeek 直连 2 个、OpenRouter 转发 6 个）
- `current` / `utility`：现在的默认是谁
- `providers`：只回答**配没配** —— 🔴 **key 一个字节都不出这里**

⚠️ **`LLMConfig` 里没有 `backend` 字段** —— 它在构建时被翻译成 `provider`/`base_url`，
而 `provider` 又是两家共用的协议名。所以**按 `base_url` 认人**（`_backend_of`）。

切换模型**不走这个接口**：Chat 请求带上 `model` 短名就行（`config.models` + `adapter_for` 那条老路）。

顺带：`/usage-stats` 补了**近 14 天日粒度**，空日子补零 —— 图要连续。

### 42.2 🔴 厂商账本：页面上终于有能称为「真」的钱

DeepSeek **没有明细账单 API**（只有余额接口），OpenRouter 的 credits 接口
给累计真实消耗。所以**真账走快照差值**：

```text
PROVIDER_LEDGERS  一个服务商一个 fetcher
  起来记一笔 → 之后每 6 小时记一笔 → 存进 ledger_balance 表
  真实日消耗 = 相邻两行快照的差值
```

- 读账时若最新快照**超过 1 小时旧**就顺手补一笔，**不阻塞**读。
- 🔴 **DeepSeek 的余额差要加上 `topped_up` 增量**，
  不然**充值那天会显示成倒赚**。
- 没配 key 的账本**安静地空着**；新服务商加一个 fetcher，账本自己长出来。
- `usage-stats` 多了 `providers` 块（余额 + 真实日消耗 + 该厂商流水估算）。
  **两本账并排放**：`token × 单价` 那套估算照旧留着，别混成一个数。

### 42.3 Studio 两间房：工具墙 + 插口面板

| | |
|---|---|
| `GET /api/nox/tools` | loop 里注册的 `ToolSpec` 全量吐出（名字 + 给模型看的那句描述），`None` 槽位跳过 |
| `GET /api/nox/integrations` | `cfg` 里的 **11 个插口**逐个探活 |

- 分组**前端按名字前缀认**，后端不维护第二份分组表。
- 探活语义：**对方应一声就算连着**（401/404/405 都算「在」），连不上 / 超时才算断。
- 🔴 **首版上线当场栽在这**：`urllib` 会把 404/405 抛成 `HTTPError`，
  **必须单独接住算通** —— 不然 MCP 端点全被冤枉成断线
  （MCP 对 GET 回 405 是正经行为）。
- 结果**缓存 60 秒**，探活 8 线程并发 —— 探一轮要几秒，不能每次进页面全量打一遍。

### 42.4 Notifications 页：闸的安静时段 + 订阅清单

- `gate.dump_state` 补 `quiet_start` / `quiet_end`：
...
## 四十六、点单确认卡 + 反向链路可见性（2026-09-06 一日落地）

> 设计文档：`Caelum-点单确认卡-设计.md`（含 previewOrder / createOrder 的真实返回结构、
> 卡片二被现实推翻两次的过程、淘宝结案）。本节只记工程侧的现状与坑。

### 46.1 下单从「模型说了算」变成「她点了算」

**病根**：这批国内 MCP 的「确认制」全靠工具描述里那句话。`tools/mcd.py` 的
`ACTION_TOOLS` 常量**定义完之后整个仓库没有任何地方用到**，测试也只断言
「描述里有『确认』二字」——验的是那句话写了没有，不是确认真的发生了没有。

**改法**：`luckin_order` 工具**不再下单**，只写 `orders.db` + 吐一张确认卡；
真正的 `createOrder` 挪到 `POST /api/nox/orders/{id}/confirm` 后面 ——
**模型物理上够不到它，因为它不在工具表里**。这是把提示词变成代码。

```
出卡（对话流里）   luckin_order → 自己调 previewOrder 拿权威价 + queryShopList 拿候选门店
                              → 写 orders 表 → attachment
确认（新的 HTTP）  前端 → bridge 代理 → nox-core：claim() 幂等 → 重跑 preview 比指纹
                              → 真调 createOrder → 存 pay_link（单独表，带 TTL）
```

- **新库 `orders.db`**（第五个）。不并 world.db：那边契约是「Observation 永不改写」，
  订单状态机天生要改。不并 sessions.db：会话删了订单还在
- **幂等靠 `claim()`**：`UPDATE … WHERE state='pending_confirm'` 让 SQLite 保证只有一个请求能成。
  先查后改的话她连点两次会**下两单**
- **指纹绑定**：确认时重跑 preview 比对，价格或券变了就拒绝重新出卡
- 8 个状态（调研文档列 12 个，多的是支付宝那套预算预占/对账，不碰钱就不需要）

**那 10 块钱**：她花 20 买了本该 10.9 的生椰拿铁。`previewOrder` 会**自动挑好券**
（`couponCodeList` 直接给结果），他没调 preview，`createOrder` 也就没带券。
现在**价格和券由工具自己调 preview 取，不问模型要** —— 模型转述价格这条路一旦留着，
就永远会有一次它转述错了而没人发现。

### 46.2 🔴 三次「运行期才炸」，同一类

当天连续踩三次，`py_compile` 和工具层单测**一次都没拦住**：

| # | 错 | 表现 |
|---|---|---|
| 1 | `api/server.py` 用了 `luckin_order_flow` 但没导入 | **订单在瑞幸真建了**，下一行 NameError → 前端显示「失败」。商家那边成了、我们这边以为没成 |
| 2 | 检查函数闭包引用 `history`（流式端点的局部变量） | 写的时候发现，差点重犯 |
| 3 | 同一段漏 `import re` | 同上 |
| 4 | 晨检脚本漏 `import re` | 那块静默走兜底，显示「手·查不到」而不报错 |

**补的防线**：一条**真的把 `/chat/stream` 跑起来**的测试。
验证过它有效：删掉 `import re` 重跑 → 红 6 条。

> **`py_compile` 只查语法，名字查找是运行期的事。**
> 唯一能拦住这一类的是真发一次请求。

### 46.3 他说「卡发你了」却没调工具

当天连着两轮。**她等一张永远不来的卡，而日志干干净净、一个错都没有。**

病根两层：
1. **我在工具描述里写了台词**「说一句『卡发你了，你看一眼』就够」——
   等于教他「说这句 = 完成任务」。`context/providers/resonance.py` 早写过
   「写台词的话，出来的是模板」，我写的是它的**动作版**，更凶
2. 那两轮进了历史，他开始**照抄自己**。全新会话里同样的请求工具链完整
   （`amap_search_poi → luckin_shops → luckin_search → luckin_preview → luckin_order`）

**⚠️ 我最初建议「开个新会话」——错的**：Caelum App 是单一窗口、无 Recents，她换不了。

所以只能在同一条历史里掰：检测到「声称发了卡但这轮没产出卡片」→ 记进
`core.card_debt` → 下一轮 `_dynamic()` 在他的动态块里当面点破 → 真发出卡就销账。
进程内存不落盘（重启就忘，那正好）。三条测试盯着描述和返回里不许再出现能照抄的句子。

### 46.4 反向链路：断线根因 + 让她能看见

**根因**：`uvicorn.run(...)` 没传 ws 参数，吃默认值 ping 20s / **timeout 20s**。
服务端每 20 秒 ping，20 秒内收不到 pong 就关连接 —— 家用网络抖一下超过 20 秒，
这条链就没了。改成 ping 20s / **timeout 60s**。

排查时**先把断开码分开**（这一步最花时间）：

```
(1012) Service Restart  × 21   全是当天部署重启，不是故障 ← 混进统计会天天误报
(1000) 正常关闭         × 107  全挤在 09-05 23:18–23:19 两分钟里 ← 见下
(1005) 无关闭帧异常断开 × 8    这个才是她遇到的
```
最糟一次 12:06:23 断、12:19:39 才回来，**13 分钟**。

**新发现的故障模式：两个网关在抢。** 107 次「正常关闭」约每秒一次 ——
`_handle` 里「新设备连上，顶掉旧链路」遇上**两个 local-gateway 同时活着**就死循环：
A 顶掉 B，B 退避 1 秒重连顶掉 A。
🔴 **这种故障长得像一切正常**：链路一直显示「连着」、灯是绿的、日志全是正常关闭。
现在一分钟重连 ≥5 次判定在抢，日志 WARNING + 前端**优先于「连着」**显示成琥珀色。
不自动断开谁 —— 从 VPS 这端分不出哪个才是她要的那个网关。

**让她能看见**（位置是她指定的）：
- `GET /api/nox/link`（bridge 代理）：连着没有 / 多久 / 设备 / 本次运行断过几次 /
  一分钟重连几次 / 最近 20 条连断事件（带「断了多久才回来」）。**存内存不落盘**
- **OS 左侧栏**（她说不要顶部）：挂进现有的 `Nox's Status` 卡，点一下跳 Advanced
- **Settings → Advanced**（菜单项本来就在，之前是占位）：链路详情 + 断线时间线 + 服务体检
- **晨检卡加第五块**：`手✗断8次，最久 3 分`。**只数 1005/1006，不数 1012**

两条纪律写死在实现里：
1. **只报事实不猜原因** —— 从 VPS 这端分不出是进程退了/网切了/电脑睡了。
   当天他就猜了这三种，猜错的话她会照着错方向排查
2. **三种状态不能混成红绿两色**：问不到 core（灰）/ 没配链路（不显示）/ 真断了（红）。
   混在一起的话 core 挂掉会被读成「她电脑没开」

> 顺带：`loadHealth` 这个名字已经被**她手环的健康数据**占了，
> 服务体检另起 `loadServiceHealth`。两件不相干的事共用一个名字，迟早有人调错。

### 46.5 现状与回滚

- 瑞幸走确认卡；**麦当劳维持现状**（提示词确认制），做完一家把坑踩完第二家才是填适配器
- 淘宝**决定不做卡片**（连商品展示卡也不做），见设计文档第十一节
- 回滚点：`/root/nox-core/*.bak-0906*`、`/root/bridge/server.js.bak-0906*`、
...
### 47.7 她测完之后（2026-09-07 当天返工）

糖糖真机打了一通，报了三条。**三条的根因全不在我以为的地方。**

> 「在电脑端的语音和手机端是不太一样的，手机端的主要场景是电话陪伴。
> 电脑端的主要场景是操作读写指令。」

这句话是这次返工的总纲 —— 我上一版把两个场景当成同一件事，参数、厂商、时序全是照陪伴调的。

#### ① 「音色会跑偏，一段语音里出现两种声音」——不是音色问题

分句流水线把一段话切成好几次**独立**的 `POST /api/tts`，而降级链是**逐请求**的：
任何一句 v3 失败就换 turbo，再失败换 edge-tts —— **edge 是完全另一个人**。
加上 `eleven_v3` 的 `stability: 0.34` 很低（表现力强＝随机性大），
就算全都成功，逐句独立生成本来就会在句间跳。

查证到的死路：**eleven_v3 既不支持 request stitching，也不支持标准 TTS WebSocket**。
只要还是「v3 + 分句」，就没有任何办法让相邻两句保持同一把声音。

她的裁决：「不用 ele 家的，换一个中文识别更好的，ele 的中文不好，
再去掉情绪更不好了。」—— 所以不是在「情绪」和「一把声音」之间二选一，是**换厂商**。

- 桌面 TTS 换 **阿里 qwen3-tts-instruct-flash**：中文原生；`instructions` 吃中文自然语言
  指令（「用男朋友在电话里说话的语气…」），比 `[softly]` 那套标签还细 —— **情绪没丢**；
  而且 **DashScope 的 key 我们本来就有**（ASR 在用），不用她开新账号
- **一轮之内粘住一个引擎**：前端把上一句用的引擎回传，bridge 优先用它，
  真失败才降级，最多换一次
- **把用了谁说出来**：响应头 `X-TTS-Engine` + 通话面板一行小字（降级时标黄）。
  「优雅降级会把故障藏起来」（二十二节）这是第二次应验，这次留了出口
- ⚠️ 阿里回的是 **wav 不是 mp3**：前端原来写死 `audio/mpeg` 建 Blob，
  照搬会得到**一段静音且不报错**（onerror 走的是 resolve）。现在按响应的
  Content-Type 走，wav 不喂 MediaSource（`isTypeSupported` 是 false）

#### ② 「我说的话会被截断」——我把手机的参数原样搬到了桌面

判定「她说完了」用纯静音时长 600ms，**那是在她手机上按陪聊调的**。
口述指令时她会在句中想词（「帮我在本机…写个…架构文档」），600ms 一到就提交。
音量阈 0.045 同理，是手机麦克风的数。

我在 `tuning.js` 顶上写过「唯一一份」—— 一份是对的，但**一份不等于一个档**。

但**光把阈值调大是拿一个抱怨换另一个**：她同时也在抱怨他慢。
静音时长这一个信号里没有「她说完没有」这个信息。所以：

- 分档 `PROFILES`：phone(600/1100) / desktop(800/1800)
- 新增 `turn.js`：**穷人版转折点判断**。看实时转写**断在哪个词上** ——
  「帮我在本机」「把那个文件」显然还有话（话头开着、动词没出来），
  「文档写好了」「睡了」就是说完了。听起来悬着就等 patient 那个数，
  否则用短的。**该快的时候快，该等的时候等**
- 两个 VAD 打架：bridge 给 dashscope 开的 `server_vad` 写死 400ms，
  **比客户端任何一档都急** —— 前端调了等于没调。现在由前端传
  `silence_ms`，而且传的是**最长的那个**：服务端只当安全网，
  什么时候算说完由客户端判

真正的解法是转折点模型，已经选好了：**pipecat 的 Smart Turn v3**
（8MB int8 ONNX、Whisper-tiny 底、CPU 12ms、BSD、看波形不看文本，
专门训练过「嗯…」这类填充词）。8MB 意味着能跑在 **Electron 主进程**里，
音频不出她的电脑。见 `turn.js` 顶上的注释。

#### ③ 「弹窗慢一拍，我说到下一个问题了他才操作上一个」——打断没有真的中断

她插话时前端 abort 了自己那截 SSE，但 **bridge 到 Core 的上游连接完全没断** ——
Core 那一轮照跑完，工具照调、审批弹窗照弹。**那不是延迟，是错位。**

现在 bridge 在 `res` 关闭时 abort 上游。Core 那边会在下一个 `yield` 上收到
GeneratorExit —— 而 47.3 加的 `tool_start` 恰好是每件工具**执行之前**的那个 yield，
所以最坏是跑完手上这一件就停。被打断的半句**两边都不落库**（Core 也走不到
`sessions.put`），状态一致；严格说该「只记她真正听到的那半句」，
但只有前端知道播到哪儿了 —— 记全文会让他下一轮引用一句她根本没听见的话。

#### ④ 顺手挖出来的两个洞（她没报，但都是哑的）

- **桌面根本连不上国内那条实时中文识别**：`desktop/main.js` 的 `PROXY_PREFIXES`
  写着 `/ws`，但那个本地服务**只接了 `request` 事件，没接 `upgrade`** ——
  Node 默认把连接销毁。不报错，就是一个永远连不上的 WebSocket。
  补了 TLS 隧道转发；vite 那边也补了 `ws: true`
- **阿里那条 STT 从 batch 起步就永远起不来**：`connectStt()` 开头有一句
  `if (sttMode === "batch") return;`。以前没暴露是因为默认走 Scribe（它自己会
  把模式扳成 realtime），阿里只在手动切换时用，那条路恰好显式扳了一下。
  桌面默认走阿里，洞就露出来了

#### 现状

- 桌面：STT 走国内实时（qwen3-asr-flash-realtime）、TTS 走 qwen3-tts、desktop 档
- 手机：一个字没动（Scribe + eleven_v3 + phone 档），陪伴场景保持原样
- ⚠️ **qwen3-tts 的调用形状我在本机验不了**（key 只在 VPS）。
  音色名尤其不能信文档抄来的那个 —— 音色和模型版本要配套，名字错了是
  **200 但没有 url**，又一个静默失败。所以写了 `scripts/tts-audition.mjs`：
  在 VPS 上跑，把候选音色各念一句她真会听到的话，写进 uploads 打印链接，
  **她的耳朵选**，选完写 `QWEN_TTS_VOICE=` 重启 bridge

#### ⑤ 当天的返工的返工：差点把手机也一起换掉

上面那条「桌面换阿里」，我第一版是把 `["qwen", ...]` 写成了 **`/api/tts` 的全局默认**。
后果不是桌面变好，是**手机也被切走了**：

- 手机通话的第一句不带 `engine`（那是「这一轮已经用上谁」，第一句当然没有）
- 聊天里的语音条（`Chat.jsx` 的播放按钮）**从来不带任何参数**
- 于是两条都落到 qwen 上；英语陪练更糟 —— 降级路径传的是剥净标签的 `clean`，
  英文句子会被中文优先的模型念、`[whining]` 那套全丢。
  **和 2026-08-15 修掉的「edge-tts 用中文发音人念英文」是同一种错，隔三周我又犯一遍**

而且我在交付说明里写了「手机端一个字没动」——**那句话是错的**。
是糖糖看完问「手机端也降级成 qwen3 了？」才发现。

改法和教训：

- 顺序**按端分**，前端明说自己是 `desktop` 还是 `phone`；
  **不带 `profile` 一律按 phone 算** —— 老调用方的默认必须是「和今天之前一样」
- 上一版的三个 `if (chain.includes(...))` 是结构性错误：**顺序被写死在代码结构里**，
  chain 只能决定跳过谁、决定不了谁先来。改成按 chain 顺序循环
- 抽成 `bridge/lib/tts-chain.js` + 7 条断言（`test/tts-chain.test.js`）。
  第一条就是「不传 profile 必须还是 eleven-v3 打头」

> 📌 教训：**换默认值 = 改所有没指定的人的行为。** 加一个更好的东西时，
> 「让它当默认」和「让想要的人能选它」是两件事 —— 前者会波及所有
> 老的、不带参数的调用方，而那些调用方往往正是最不该动的那些。

#### ⑥ 回滚：qwen 整条退出（2026-09-07 当天）

糖糖：「app 没改过来。你先回滚到之前。」

**语音合成回到今天之前的样子** —— 两条链都是纯 ElevenLabs 三级降级
（v3 → turbo → edge），`qwen` **不在任何一条链上**。她手机上听到的
和昨天一模一样。

- `bridge/lib/tts-chain.js`：两条链都去掉 qwen；测试第一条改成
  「qwen 不许出现在任何一条链上」—— 要重新打开，**先改链再改测试**，
  顺序反了就等于没人守着
- `shared/voice/tuning.js`：手机那一档的 `silencePatientMs` 拉回和
  `silenceCommitMs` 相等 = **转折点判断对手机不生效**，时序和今天之前一致。
  陪伴场景本来就没报过被截断 —— **没坏的地方不要顺手改**
- ~~阿里那条路的代码全留着（`qwenTts()`、试听脚本），一行就能开回来~~
  → **2026-09-09 整套删了**，见 47.8。这条留后路的做法本身就是问题

⚠️ 手机端**还剩两处和今天之前不同**，回滚时保留了，因为都是她点头要的：
通话页改用了 `shared/voice/` 那份共用引擎（她定的「整块抽到 shared」），
以及语音前缀多了两句工具约束。真要连这些一起退，
`git checkout HEAD -- frontend/src/pages/VoiceCallDesktop.jsx` 一条命令的事。

> 📌 教训（第二次写进这一节了）：**「代码写好了」和「该当默认」是两件事。**
> 这次是我把新厂商当了默认，波及了所有不带参数的老调用方；
> 回滚时同样要克制 —— 只退真正出问题的那一层，别顺手把她要的东西一起退掉。

#### ⑦ 上线记录（2026-09-07 晚）

**bridge**（备份 `/root/bridge/server.js.bak-0907-before-rollback`）
- 新增 `lib/tts-chain.js` —— **这个文件必须一起传**，`server.js` import 它，少了起不来
- 线上实测链子：`{"desktop":["eleven-v3","eleven-turbo","edge"],"phone":[同上]}`，qwen 一条都不在
- 日志坐实了回滚前的状况：`[TTS] qwen3-tts 449324B / 542ms` —— 她手机上听到的就是它

**前端**（旧的留在 `/root/frontend/dist-old`）
- 线上 `index-D5ZPpeKI.js` + `index-BZb-5hJK.css`，和本地当前构建一致
- `/home/house.webp` → 200 / 264KB（此前是 404 再拉 2.4MB 的 png，首页那个破图就是这么来的）

**nox-core**（三个文件各留 `.bak-0907`）
- `agent/loop.py`、`agent/llm.py`、`personality/scenes.py`
- ⚠️ **传完先用线上的 3.10 编译再重启**（本地是 3.12，这条纪律见部署那节）
- 修掉一个半截状态：`api/server.py` 早前跟着别人的改动上去了（有 `tool_start` 的接收端），
  而 `loop.py` 还停在 7-28（没有发送端）—— 接收端一直是哑的

**端到端验证**（`test-` 前缀沙盒会话，验完 `DELETE /session/` 清掉）：

```
POST /chat/stream {"voice":true,"scene":"zh","text":"现在几点了"}
→ "type":"tool_start","tool":"get_current_time"
→ "type":"tool_end","tool":"get_current_time"
→ 16 帧 text → done
```

**语音路径上工具进度真的通了** —— 不是「代码写好了」，是线上跑出来的。

#### ⑧ 首页底部那条浅带：bgBase 只该在顶部（2026-09-07 晚）

糖糖发了两张对比图：dreamy 看不出，warm 的最下面有一条浅色带子
（「最下方，主题是铺不到的」）。

**根因不在 Home 的 CSS，在启动脚本。** `index.html` 的内联脚本按
`localStorage` 里的主题把 `theme-color` 和 `html` 底色设成该主题的 **`bgBase`**
—— 而 bgBase 是**渐变最顶上那一档**（warm `#F4F2EE`、dreamy `#fdf0f6`）。
iOS PWA 的视高比屏幕矮一截，底下漏出来的就是它：

- dreamy：渐变末端也浅 → 挨着，看不出来
- warm：渐变末端是深棕 → 一条浅带子，刺眼

同一个机制也解释了她说的**「每次要杀掉进程才会同步当前主题色」**：
`themes.js` 运行时确实改了 `meta[theme-color]`，但 iOS 那块壳色**只在启动时读一次**，
不重画；重启后启动脚本按新主题定色才对上。不是没同步，是它只在启动时听。

> 📌 所以 `0ff96ee`（状态栏改 `black-translucent` 沉浸式）那次定下的规矩是：
> **bgBase 只出现在顶部** —— 那儿它和页面第一档同色，天然融合。
> 底部只要渐变往深走就必然穿帮。

改法（两层，都不占页面像素）：

1. 渐变从**滚动容器的 background** 搬到 `position:fixed; inset:0` 的层
   —— 滚动/橡皮筋回弹时容器背景跟不上，这层永远盖满一屏
   （⚠️ `z-index` 只能 `0` 不能 `-1`：父级只有 `relative`、不构成层叠上下文，
   给 -1 会掉到 `#app` 的底色后面，整片渐变消失）
2. 再垫一条 `bottom:-200px; height:200px` 的**末端色**带，从视口下沿往下 ——
   不管 iOS 那一截有多高，露出来的都是页面自己的颜色

实测（warm 主题）：渐变层 `[0,600]`、兜底带 `[600,800]`、末端色
`color(srgb 0.497 0.252 0.158)`，页面内零占用。

### 47.8 阿里 TTS 整套下线（2026-09-09）

47.7 ⑥ 那次回滚只把 `qwen` 从链上摘掉，代码全留着、写明「一行就能开回来」。
两天后糖糖看到这段，说了一句把事情定死的话：

> 「我没听过阿里的音色，很不好听。我还没决定用什么 tts 那个会话就直接用了阿里。」

也就是说 09-07 那次**根本不存在「她同意换」这个前提** —— 是先换了、她听见了、
才有了后面的回滚。那么留一条「一行就能开」的后路，等于把「要不要换她的声音」
这个决定继续搁在代码里等一个开关，而这个决定从来不属于代码。

删掉的：`qwenTts()`、`QWEN_TTS_URL/MODEL/VOICE/INSTRUCTIONS`、`tryQwen()`、
链路分发里的 `if (name === "qwen")`，以及桌面端通话面板把 qwen 当正常引擎显示的分支。

**没动的**（同名不同事，别顺手删）：
- `DASHSCOPE_API_KEY` —— 语音识别还在用，删了她说话就没人听得懂
- `qwen3.5-flash` —— 那是他的眼睛（第十九节），和 TTS 无关

守门的：`bridge/test/tts-chain.test.js` 两条 ——「阿里不许出现在任何一条链上」
+「server.js 里也不许留着阿里 TTS 的代码」（后一条读源码文本，认的是
`qwenTts` / `QWEN_TTS_` / `tryQwen` 这几个标识符，不是「qwen」这个词）。

> 📌 教训（这一节第三次写）：**「跑通了」「该当默认」「她要不要」是三件事。**
> 前两件我能判断，第三件只有她的耳朵能判断。感官类的选择 —— 音色、配色、语气 ——
> 在她试过之前，连「默认」的资格都没有，更不该留一个等着被打开的开关。

## 四十八、没人看的日志 + 被挤掉的待办（2026-09-08）

糖糖那句：

> 「话说我们现在有日志了，有警告，没人看是个问题吧？这个出问题第一时间
> 看看能不能先报给 nox，让他理清楚，但是先不让他自己操作。」

那天量出来的代价，三件全躺在日志里、一条都没被看见：

| 事故 | 持续 | 为什么没人发现 |
|---|---|---|
| utility 全线 401 | **30+ 小时** | 主聊天照常，表面完全正常 |
| 「未知型号按 Sonnet 估价」 | 3 天 8728 次 | 账单虚高 27 倍，但页面照常显示 |
| galatea 接线 TypeError | 崩溃循环 | 部署窗口里刷屏，事后没人回看 |

### 48.1 日志摘要（`vps-scripts/log-digest.py`）

每天随晨检跑一次，扫 7 个服务 48 小时的 journal，几千行 → 57 个类目。

四条设计，每条都有对应的翻车：

1. **只报新增和突增，不报存量。** 每天原样铺 57 类，三天后她就不看了 ——
   那就退化成另一种墙纸，和躺在 journal 里没区别
2. **必须有静音名单**（`vps-scripts/log-digest-mute.txt`），否则第 1 条迟早被噪音绕过
3. **不用 LLM。** 那天挂的正是 utility ——
   **一个和被监控对象共享故障模式的监控不是监控**
4. **每条带「最后一次多久以前」。** 第一版没有，我当场就被自己坑了：
   看见 register_all 崩了 60 次冲过去查，查完发现是三小时前部署窗口里的、早好了

出口两个（**只看不动**，她划的线）：
- 晨检卡加一句结论（body 硬卡 260 字，塞不下细节）
- Caelum OS → Settings → Advanced 一张「日志」卡，细节在那儿
  —— 「没人看日志」的解法不能是「再加一个要看的地方」

### 48.2 🔴 它自己把 VPS 干趴了

第一版用 `subprocess.run(capture_output=True)` —— **把整份 journalctl
输出一次性读进内存**，`.splitlines()` 再翻一倍。nox-core 48 小时几十万行。

内核 OOM 杀掉一个 `anon-rss:370986kB` 的 python3，systemd-journald
被拖到 watchdog 崩溃，整机 SSH/HTTP 全不通，只能去云控制台硬重启。
那台是 4GB 而且当时 `Total swap = 0kB`。

**一个为了「早点发现问题」而写的东西，成了当天最大的故障。**

修法三道：流式逐行读 + 30 万行硬闸 + `RLIMIT_AS 256MB` 自限。
实测峰值 **63MB / 1.65 秒**。第三道守的是下一个 bug ——
撞上限的话它自己 `MemoryError` 死掉，晨检卡少一行，而机器还活着。

事后加了 2G swap。

### 48.3 顺手挖出来的：他每天几十轮看不见她的待办

扫描器第一次跑就报了一条没人注意过的：
`Context 超出 800 字预算，这轮略过: todo` —— **48 小时 61 次**。

原来那条警告只说「丢了谁」，不说「谁吃掉的」，所以查不下去。
先把它改成能回答问题的：

```
略过: todo(252字)（已用 604，吃得最多的：memory 266、mood 167、health 97、location 43、time 26）
```

一行说清：**604 + 252 = 856，只超 56 字**。没有哪个 provider 畸形，
是 **2026-09-05 MemoryProvider 解禁往每轮加了 266 字，而预算是解禁前定的**。

三处一起改：

1. **`todo` 渲染统一裁法**（`context/providers/todo.py`）——
   `due_soon` 原来「每条一整行、原文照搬、不限条数」。走 GitHub 时它只收
   今明两天到期的（一两条）没事；2026-08-18 换本地清单后它装的是
   **整个「进行中」分区**，换源只改了取数没改渲染。
   合成数据实测 981 → 116 字（她当前真实清单 267 → 252，
   所以这条守的是极端情况，不是当下主因）
2. **预算 800 → 1200**（`context/registry.py`）。代价按 glm-5.3-flash
   未命中价 0.8 元/M、80 轮/天算 ≈ **0.5 元/月**
3. **`todo` 排到 `memory` 前面**（`router/intent.py`）——
   **顺序就是丢弃顺序**。她今天要做什么是事实，记忆是背景
   （memory.py 自己写着「这些是背景，别刻意提起」）。
   装不下时宁可少一段背景

### 48.4 这次学到的三条

- **测试覆盖的是没在跑的那条路。** todo 所有测试走 GitHub 路径，
  而线上早就走本地清单了 —— 所以「不裁不限」一直是绿的。补了 5 条本地路径测试
- **没验过会不会红的测试等于没有。** 新测试写完把渲染临时改回旧版，
  确认三条真的红、旧代码真的渲染出 981 字，才算数
- **查不下去的警告 ≈ 没有警告。** 61 次警告没人能顺着查，
  改成带「谁吃掉的」之后一行定位

## 四十九、Caelum Moments：他的自我表达层（2026-09-15，**默认 off**）

设计全文 `Caelum-Moments-设计.md`（第七节是验收标准，第十节是这次的任务拆分表）。

### 它不是「多一个功能」，是给一层已经存在的内心状态开一个不打扰的出口

系统里**早就有一整层「够不着开口阈值」的东西，而且每一条都是故意压下去的**：

```
attention/playfulness.py:24        「她心情好不该换来一次开口」
attention/regret.py:31             「故意压在开口阈值之下」
attention/sources/curiosity.py:36  ≤0.45，「不该变成一次打扰」
attention/appraisal.py:223         规则版强度全部压在 GENERATE_THRESHOLD(0.55) 之下
```

线上实测（`/api/nox/resonance`，2026-09-14）：longing 0.40 · playfulness 0.28 ·
regret 0.167 —— **全都够不着 0.55**，理由都一样：不该变成一次打扰她。
Moments 接的就是掉在阈值底下的那些。

### 形状：内心 × 时机，**两边都得有**

`moments/impulse.py` 是**纯函数**（无 IO、无时钟、无随机，所以能穷举测）：

```
inner  = combine(drives)              叠加，不是取最大（抄 resonance 的语义）
timing = (今天聊得少 + 她多久没说话 + 距上次发帖多久) / 3
value  = inner × timing               任何一边是 0，冲动就是 0
```

`THRESHOLD = 0.45` 是**挑出来的不是拍的**：它比线上任何**单独一条** drive 都高，
所以「只有一件心事、哪怕时机满分」也发不出去 —— **必须有第二件事同时压着**。
这就是「不是规则机器人」在数值上的落实，也是 `if drive == "longing": post` 这种
写法被结构性挡住的地方。

### 三道和「开口」划清界限的闸

- **R10（新法则 + 哨兵）**：产出路径里不许出现 `api/push/send`。
  发帖不推送、不弹锁屏、不占 Care 每日额度 —— 它不是开口。
  哨兵口径认**完整端点路径**，故意不认裸的 `push/send`（否则会被解释这条法则的文档触发）；
  外加 **R10b** 守着「`moments/` 目录还在」——**空集不是通过**。
- **`ledger.POSTED`**：账本照记（R1 的精神：为什么说话要能沿账本溯源），
  但**不算进 `considered = spoke + skipped + blocked`**，也**不开 Care 链**。
  🔴 设计文档原本写的是「照抄 `note_external_speech`」——**照抄是错的**：
  `summary()["spoke"]` 是 `longing.tick(spoke_today=)` 的入参，算成开口的话，
  表现就是「他发了条朋友圈，于是不那么想她了」。所以另起 `note_moment`。
- **`MomentRecord` 的结构闸门**：`mode="shadow"` 且 `posted=True` **构造不出来**
  （`__post_init__` 抛）。「shadow 里偷偷落了真帖」是这个功能最坏的一种坏，
  不靠 loop 里记得写个 `if`（抄 `temporal/result.py` 的做法）。

### 节奏

一天最多 2 条、两帖至少隔 3 小时、**不设安静时段**（深夜发帖不吵醒任何人，
而且深夜的帖子才有味道）。过阈值之后还要掷骰子（`P_MAX=0.15`/tick）——
**随机不是随机抽时间，是随机发生在有理由的内在状态上**。计数和 `last_post_at`
落 `source_state`（跨重启）；跨天**只清 count，不清 `last_post_at`**，
否则昨晚 23:50 发过、今天 00:05 还能再发，MIN_GAP 在零点开一个不报错的洞。

### 上线两步走（**别合并**）

1. 先发代码，`NOX_MOMENTS=off` —— 零行为变化（真跑过 lifespan 验的，不是读源码）
2. 开 `shadow`：只算冲动、只记日志、**不落帖**，看它一天想发几条、为什么
3. 节奏对了再 `on`

shadow 的记录是三段式（输入快照 / 算出来什么 / 发没发和为什么），
**缺一段构造不出来** —— 理解层那次 shadow 跑了一周多、回头什么结论都给不出，
就是因为只记了数字没记「为什么」。

### 这次的验收是怎么做的

**每一块都真的改坏过**：bridge 3 次、impulse 4 次、record 6 次、writer 9 次、
loop 8 次、接线 5 次、账本 4 次、哨兵 4 次（含一条**反向**变异：改了不该红的东西，
确认它**不红**）、桌面页 2 次、手机页 3 次 —— 一共 48 次，全部红在该红的那条上。
