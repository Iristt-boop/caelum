...
// ---------------------------------------------------------------- 阅读状态
//
// Phase 1（架构设计 v1.2 第 4.1 节）。和文档略有出入，理由写在这儿：
//
// 文档说往 progress.json 里加 status / mood / openQuestionIds 三个字段。
// 实际只有 **mood** 需要落盘 —— 另外两个都能从现有数据现算：
//
//   status          ← lastReadAt + 这本书的批注时间
//   openQuestions   ← 她提了问、他还没回的那些批注
//
// 落盘的派生字段迟早和真值对不上（`chunksRead` 已经有过一次前科），
// 而且要在 markRead / annotatePassage / replyToAnnotation 每处都记得更新，
// 漏一处就是错的。现算的成本几乎为零：manifest 和 annotations 都有缓存。

/** 最近这么久内有动作就算「正在读」 */
const READING_WINDOW_MS = 30 * 60 * 1000;
/** 超过这么久没动就算「放下了」 */
const PAUSED_AFTER_MS = 3 * 24 * 60 * 60 * 1000;
/** 同一章堆到这么多条未闭合的批注，就当她想聊了 */
const WANTING_TO_TALK_NOTES = 3;

function isQuestionNote(note) {
  const text = String(note?.note || "");
  return /[?？]/.test(text) || /(为什么|怎么|如何|是不是|能不能|什么意思)/.test(text);
}

/**
 * 算出这本书现在是什么状态。
 *
 * 判定顺序有讲究：finished 优先于一切（读完了就是读完了，不管多久没碰）；
 * paused 排在 thinking 前面 —— 三天没动的「在回味」不叫回味，叫放下了。
 */
function computeReadingStatus(manifest, entry, bookAnnotations, now = Date.now()) {
    const readIds = validReadIds(manifest, entry);
  if (manifest.chunks.length > 0 && readIds.size === manifest.chunks.length) {
    return "finished";
  }

  const lastReadAt = entry?.lastReadAt ? Date.parse(entry.lastReadAt) : NaN;
  const sinceRead = Number.isNaN(lastReadAt) ? Infinity : now - lastReadAt;

  // 她最近写的批注（不含他的回复）
  const hers = bookAnnotations.filter((a) => !a.parentId && isHumanAuthor(a.author));
  const lastNoteAt = hers.reduce((max, a) => {
    const t = Date.parse(a.createdAt || "");
    return Number.isNaN(t) ? max : Math.max(max, t);
  }, 0);
  const sinceNote = lastNoteAt ? now - lastNoteAt : Infinity;

  if (sinceRead > PAUSED_AFTER_MS && sinceNote > PAUSED_AFTER_MS) return "paused";

  // 同一章里她还没得到回应的批注攒够了 —— 她想聊
  const answered = new Set(
    bookAnnotations.filter((a) => a.parentId && !isHumanAuthor(a.author)).map((a) => a.parentId),
  );
  const byChunk = new Map();
  for (const a of hers) {
    if (answered.has(a.id)) continue;
    byChunk.set(a.chunkId, (byChunk.get(a.chunkId) || 0) + 1);
  }
  for (const count of byChunk.values()) {
    if (count >= WANTING_TO_TALK_NOTES) return "wanting_to_talk";
  }

  // 刚写过批注、但没继续翻章 —— 停在那儿想
  if (sinceNote < READING_WINDOW_MS && sinceNote < sinceRead) return "thinking";

  // 到这儿说明：没读完、没搁置超过三天、也没在憋着想聊。
  // 那就是「还在读这本书」—— 哪怕今天还没翻开。
  //
  // ⚠️ 这里原本 return "paused"，等于隔一天没碰就判定放下了，
  // 而 paused 的定义是三天（上面已经判过）。中间那段必须归 reading，
  // 否则早报会对着一本昨天刚读过的书说「这本你放下很久了」。
  return "reading";
}

/** 她提了问、他还没回的那些批注。Phase 5 的 Chapter Insight 做出来之前，先用它当 open questions。 */
function computeOpenQuestions(bookAnnotations) {
  const answered = new Set(
    bookAnnotations.filter((a) => a.parentId && !isHumanAuthor(a.author)).map((a) => a.parentId),
  );
  return bookAnnotations
    .filter((a) => !a.parentId && isHumanAuthor(a.author) && !answered.has(a.id) && isQuestionNote(a))
    .map((a) => ({
      id: a.id,
      chunkId: a.chunkId,
      question: a.note,
...
// "nox" 是他现在用的作者名（nox-core/tools/reading.py:118 就是这么写的），
// 线上已有这类回复。漏了它，卡片那边的 hasClaude 会判空。
// 和 public/card-logic.js 里的同名函数保持一致 —— 两处都要改。
function isClaudeAuthor(author) {
  const value = String(author || "").toLowerCase();
  return !isHumanAuthor(value)
    && (!value || value === "claude" || value === "assistant" || value === "nox");
}
...
export async function markRead(bookId, chunkId) {
  return withWriteLock(async () => {
    ...
    readIds.add(chunkId);
    // ⚠️ 展开 current 是必须的：这里原本是整体替换，
    // 后加的字段（mood 等）会被每次「标记已读」悄悄抹掉。
    progress[bookId] = {
      ...current,
      lastChunkId: chunkId,
...
/**
 * 给一本书的进度补上算出来的那几项：status / openQuestions / 完成度。
 *
 * 拿不到 manifest（书被删了、目录坏了）时**原样返回**，不要抛 ——
 * 「这本书的进度读不出来」不该让整个 /api/progress 挂掉。
 */
async function enrichProgressEntry(bookId, entry) {
  if (!entry) return null;
  try {
    const manifest = await loadManifest(bookId);
    const all = await listAnnotations({ bookId, includePrivate: true });
    const rows = Array.isArray(all) ? all : all?.annotations || [];
    const summary = progressSummary(manifest, entry);
    return {
      ...entry,
      status: computeReadingStatus(manifest, entry, rows),
      openQuestions: computeOpenQuestions(rows),
      title: manifest.title,
      chunksRead: summary.chunksRead,
      chunkCount: summary.chunkCount,
      // 现算，不落盘 —— 存下来迟早和 readChunkIds 对不上
      progressPercent: summary.chunkCount
        ? Math.round((summary.chunksRead / summary.chunkCount) * 100)
        : 0,
    };
  } catch {
    return entry;
...
export async function getProgress(bookId) {
  const progress = await loadProgress();
  if (bookId) return enrichProgressEntry(bookId, progress[bookId] || null);

  const out = {};
  for (const id of Object.keys(progress)) {
    out[id] = await enrichProgressEntry(id, progress[id]);
  }
  return out;
}
