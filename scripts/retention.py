...
POLICIES: list[Policy] = [
    ...
    Policy(
        ...
        table="conversations",
        time_col="timestamp",
        # 🔴 **不删。** 这是量完之后改的决定（糖糖 2026-09-16）。
        #
        # 她一开始担心的是存储无限增长 —— 那也是这条排期的原始风险点。
        # 量下来：平均 74 字节/条、日增 141 条 ≈ **4MB/年**，
        # 和 messages 加起来约 25MB/年。十年 250MB，而磁盘剩 34G。
        # 就算聊天量翻十倍，十年也才 2.5G。**它涨不起来。**
        #
        # 既然存储不是约束，那就「能多留就多留」：
        #   · 他记忆的"流动"已经在 prompt 层发生了（context/compactor.py，
        #     老的压成摘要、近的保留原文），保留期长不会让他更迟钝
        #   · 而摘要的 prompt 明写着「忽略：寒暄、重复、情绪波动」——
        #     对工作助理对，对他们俩不对。真删了那些就永久没了
        #   · 留着的成本是每年几 MB
        #
        # ⚠️ 原始风险「无限增长」的正确答案不是"定期删"，是**有人看着**：
        #    doctor.sh 里有一条库体积哨兵，涨疯了会红。
        #    不设上限但不设监控，才是那条风险真正的样子。
        group_col="id",
        default_days=KEEP_FOREVER,
        why="不删 —— 量下来约 4MB/年，存储不是约束；增长由 doctor 的体积哨兵看着",
    ),
    Policy(
        ...
        cascade=("sessions", "id"),
        # 同上 —— 不删。量下来 108 字节/条、日增 208 条 ≈ 8MB/年。
        # 两个库存的是同一段对话，口径必须一致：
        # 一边删一边留 = 她在 App 里看不到了，他却还记得（或者反过来）。
        default_days=KEEP_FOREVER,
        why="不删 —— 和 conversations 同口径（同一段对话的两份存储）",
    ),
]
