...
# 🔴 认名字不认 IP（2026-09-16）。`43.133.211.140` 已经不是这台机器了 ——
# 09-15 换绑到 43.153.x，而 `286160c` 那次「全部文档与脚本跟着改」漏了这里
# （也漏了 `deploy.ps1`，那个在 `dcf1da2` 补上了）。
#
# ⚠️ 这个脚本**有**防空集的检查（下面 `$creds.Count -eq 0` 那行会红 + exit 1），
# 所以地址错了不会静默通过。但它会红在**错误的方向**上 ——
# 提示让你去查 ssh 和 /etc/nox/*.env，而真因是机器换了地址。
