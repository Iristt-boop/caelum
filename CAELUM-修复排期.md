...
- [~] 4.7 数据保留策略：先做 `usage_log` / `conversations` / `observations` 三个最大头（1 天）
      ...
      **噪音**（某一类涨得快一个数量级）。
      ...
      · **conversations —— 留 180 天，按会话删**（糖糖 2026-09-16 定）。长期记忆在 OB 里，
        删半年前的原始对话不影响他记不记得；真出事时半年的暴露面比"全部"小得多。
        ⚠️ 定的当天一条都删不掉（最早的记录是 7 月），**2027 年 1 月**才第一次咬到东西。
      ...
      谁"顺手整理"策略表都不会有东西红。
      ⏳ **剩下两件**：
      1. **定时器没装** —— 在生产库上装每周自动删除，要单独点头。
         单元文件在 `scripts/caelum-retention.{service,timer}`。
      2. **`sessions.db/messages` 没定** —— 日均 191 条原始对话正文，比 conversations 还多，
         不在原排期的三张表里但**是同一类问题**。读它的地方全是
         `WHERE session_id = ?`（按会话取），所以删老会话只影响"翻很久以前的那次对话"。
         ⚠️ 只删 messages 会留下空壳 session，要连 `sessions` 一起按会话年龄删。
