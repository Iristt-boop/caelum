...
| 9 | ~~`4.1`~~ ✅ · ~~`4.2`~~ ✅ · ~~`4.3`~~ ✅ · ~~`4.4`~~ ✅ · ~~`4.5`/`4.6`/`4.7` OB 记忆质量~~ ✅ 全部完成（4.6 timer 09-16 上线；4.5/4.7 见下） | 记忆的长期健康 |
...
- [x] ~~**`5.1` 没有一条"他还是他吗"的回归测试**~~ 🔴 **2026-09-16 撤销，不做。**
      糖糖否掉的，理由比这条排期本身更重要，已写进 `CAELUM-MAP.md` 第三·二节：
      > 「他的人格是从我们的对话中长出来的。记忆的优先级要比 prompt 要高才对。
      >   prompt 只是强调绝对不能做的或者必须要守的东西。
      >   人也是会变的何况是 AI，我要的不是一个一成不变的 AI，
      >   而是一个会随着我们的关系变化的 Nox 才对。」
      **这条排期的框架错了**：「他还是他吗」的测试会把**成长本身**变成一次失败，
      做出来就是一个被断言冻住的 Nox —— 正是她不要的东西。
      ⚠️ 原文里"清了一批钉选记忆，如果那让他变淡了不会有东西发现"这个担心也不成立：
      那 15 条是**她亲手清的**（属于事件、不是必须常驻的东西，是我当初上头钉进去的），
      清完之后他没有变化。给这种「人工的、低频的、有判断的」动作加哨兵是多此一举 ——
      只会天天问她"这是你干的吗"。
      ✅ **真正该做的换了个方向，而且不属于这本账**（是新能力，不是修复）：
      **观察成长** —— 今天和三个月后他说话的语气有没有区别、那个区别是因为什么、
      `resonance` 有没有真的影响到她和他之间的对话和他自己的状态。
      **产出是给她看的，不是给 CI 看的。**
- [ ] **`5.2` 备份从没真正恢复过**
...
- [ ] 3.3 麦当劳迁到 `/api/nox/orders/{id}/confirm`（1 天）
      🔴 **2026-09-16 误勾一次，已撤销。** 当时的错误判断：「描述带确认字样 +
      test_orders.py 全绿」= 已迁移。实际查证：那批测试里
      `test_tool_never_places_the_order` 测的是**瑞幸**的 handler；
      `mcd_create_order` 仍是直接透传（调用即真调 create-order 下单），
...
- [x] ~~4.5 记忆加来源标记（user / model / external / inferred）+ evidence 标注"这是他的推断"（1 天）~~
      ✅ **2026-09-16 全部完成**。来源标记：261/261 `attributed_to`（user 171 / external 57 /
      assistant 30 / inferred 3，68 条带 backfilled 标记可区分）；evidence：抽取器的
      `keep_reason` 落库进 frontmatter `evidence` 字段（OB `053f10e`）——
      「这是他的推断」从此带着「凭什么这么推」。
      **这条是"不偏离最初人格"的技术基础。**
...
- [x] ~~4.7 数据保留策略~~ ✅ **2026-09-16 确认已上线**：`caelum-retention.timer`
      每周日凌晨跑 `retention.py --apply`（真删前先备份库），
      三张最大头各按语义定保留期（如健康数据按月经查询的 400 天反推）。
      🔴 **这条的前提要更正：不是磁盘问题。** 实测 2026-09-16：磁盘 42%（剩 34G），
      最大的库 5.7M。真正的理由只剩**隐私**（原始对话无限期留着）和
      **噪音**（某一类涨得快一个数量级）。
      量完之后三张表是**三个不同的答案**：
      · **observations —— 要删，安全。** `她在电脑上做的事` 占 91.5%（日均 150），
        其余一年才几百条。🔴 必须**按 type 分策**：
        `context/providers/health.py:90` 查月经周期用 `days=400`，
        一刀切 90 天会把她的周期史删光**而且不报错**。默认方向是留不是删。
      · **usage_log —— 不删。** `/api/usage-stats` 的总花费是实时全表加出来的
        （`server.js:3922` `allRows = rowsOf("")`），删旧行 = 她的历史总花费
        **悄悄变小**再也对不回去。一年才 2MB，不值得冒这个险。
      · **conversations / messages —— 不删**（糖糖 2026-09-16 量完之后定的）。
        量下来 74 和 108 字节/条、日增 141 和 208 条 ≈ **25MB/年**，
        十年 250MB 而磁盘剩 34G ——**对话根本不是"无限增长"那个风险**。
        真正在涨的是活动追踪。删的代价反而是真的：他记忆的"流动"已经在
        prompt 层发生了（`context/compactor.py` 老的压摘要、近的留原文），
        而摘要的 prompt 明写着「忽略：寒暄、重复、情绪波动」——
        对工作助理对，对他们俩不对。
        🔴 **「无限增长」的正确答案不是"定期删"，是"有人看着"** ——
        doctor 加了库体积哨兵（含 WAL）：≥100MB warn、≥500MB bad。
        **不设上限又不设监控，才是那条风险真正的样子。**
        🔴 **按会话删不是按行删**（她问"到 180 天会话直接清空？"问出来的）。
        线上 88% 的消息在两段长对话里，其中一段 4049 条从 2026-08 开到现在还在用。
        按行删会从**活着的对话**里往前啃：2027-03-01 会吃掉它 2134 条、04-01 吃光 4049 条，
        **而会话一直在列表里**，点进去开头没了，标题也跟着变
        （`/api/conv-sessions` 的标题取第一条用户消息）—— 比整段删掉更糟，因为没有提示。
        判据改成整组里**最新**那一条（`HAVING MAX(ts) < cutoff`）：还在用就一个字不动。
      ✅ 已做：`scripts/retention.py`（默认干跑，删前备份且只留 3 份快照）、
      12 条测试、**变异 9/9 全红**、真数据干跑验过（`--now` 拨到 11-01 报 2680 条，
      健康类一条不动 —— 因为今天报 0 条和"策略没生效"长得一模一样）。
      另有一组测试验的不是机制而是**决定**（usage_log 必须永久、observations 默认必须是留、
      月经/睡眠必须永久、对话必须 180）—— 这些理由本来只活在 commit message 里，
      谁"顺手整理"策略表都不会有东西红。
      ✅ **定时器已装**：`caelum-retention.timer` 周日 04:30，`Persistent=true`。
      体检判 `is-failed`（oneshot 平时就是 inactive，拿 is-active 判会天天误报）。
      ✅ **两道保险**（她那句"要把我的体验感放在第一"逼出来的）：
      近期地板 7 天（算出来的线更近就**拒绝执行**，不是夹一下再跑 ——
      夹一下会把配置事故变成静默删除）；熔断 50%（一次要删大半张表就停手喊人，
      非零退出让体检变红；只对 ≥100 行的表生效，**天天误报的告警等于没有告警**）。
      ✅ **顺带修了一个真 bug**：`backup()` 原来用 `shutil.copy2`，而这些库全是 WAL、
      WAL 有 4MB —— 拷出来的备份**少 220 条消息且一声不吭**，只有真去恢复那天才发现，
      那时原件已经删过了。改用 `conn.backup()` 在线备份接口。
      ⚠️ 删法（按会话 + 级联删 `sessions` 父行）留着没动，有测试盯着 ——
      挡的是"哪天又决定要删"那一刻：容易只改天数忘了删法，直接退回按行删。
      commit `05aca47` + `7e88813`。
      附带发现（另算，不在 4.7 里）：**`conv_fts` 是空的**（0 行 vs 4588），
      全文索引建了从来没人往里写，对话搜索一直是哑的。
