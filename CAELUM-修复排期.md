...
`0.5` 那 5 个路径暗号轮换（旧值在 git 历史里，我只外置了没换值）·
`NOX_TOKEN` · 玩具 token · `0.5b` `morning-check.sh` 别再从 unit 里 grep token ·
`0.6` 绑定收口（剩 8 个）· `0.8` `/uploads` 签名 URL + STT 音频挪出公开目录 ·
她本人要做的：WiFi 密码 · 旧 VPS root 口令 + 两台旧机 · 智谱的 PowerShell 历史 · 工作机 ACL
